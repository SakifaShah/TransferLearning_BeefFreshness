...
def load_data2():
    ...
    val_data_dir = pathlib.Path('/content/drive/My Drive/MeatFreshnessIdentification/Original_Dataset/Validation')
    ...
    val_folders = os.listdir(val_data_dir)
    ...
    train_it = datagen.flow_from_directory(train_data_dir, class_mode='binary', batch_size=16, save_format='jpg', save_to_dir=train_data_dir)

    test_it = datagen.flow_from_directory(test_data_dir, class_mode='binary', batch_size=16,  save_format='jpg', save_to_dir=test_data_dir)

    validation_it = datagen.flow_from_directory(val_data_dir, class_mode='binary', batch_size=16,  save_format='jpg', save_to_dir=val_data_dir)
    ...
    for folder in val_folders:
        for file in os.listdir(os.path.join(val_data_dir,folder)):
            if file.endswith("jpg"):
                val_image_names.append(file)
                val_labels.append(folder)
                img = cv2.imread(os.path.join(val_data_dir,folder,file))
                im = cv2.resize(img,size)
    ...
    print('OK')
    return (X_train, y_train),(X_test, y_test), (X_val, y_val), (train_image_names, test_image_names)


    #print('OK')
    #return train_it, test_it, validation_it

def create_model():

    MyModel = MobileNet(weights = "imagenet", include_top = False, input_shape = (224,224,3))
    x = MyModel.output
...
if __name__ == "__main__":
    ...
    # load the data
    (X_train, y_train),(X_test, y_test), (X_val, y_val), (train_image_names, test_image_names) = load_data2()
...
with open(predictedFileDir+"MobileNet8.txt",'w') as f:
